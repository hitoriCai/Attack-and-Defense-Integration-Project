...
class Furnace():
    ...
    def _partition(self, global_list, partition_num):
        ### 随机分割
        random.shuffle(global_list)
        ###
        global_poison_ids = [tuple(global_list[i:min(i+partition_num, len(global_list))]) for i in np.arange(0, len(global_list), partition_num)]
        # print(len(global_poison_ids))
        # print(global_poison_ids[0])
        # print(fsdfsd)
        return global_poison_ids

    def batched_construction(self):
        if self.args.poisonkey is not None:
            set_random_seed(int(self.args.poisonkey))
        # print(list(range(len(self.trainset))))
        # print(self.args.poison_partition)
