...
class PGDL2(Attack):
    ...
    Shape:
        ...
        - output: :math:`(N, C, H, W)`.
    Examples::
        >>> attack = attack.PGDL2(model, eps=1.0, alpha=0.2, steps=10, random_start=True)
        >>> adv_images = attack(images, labels)
    """
    def __init__(
        self,
...
class APGD(Attack):
    ...
    Shape:
        ...
        - output: :math:`(N, C, H, W)`.
    Examples::
        >>> attack = attack.APGD(model, norm='Linf', eps=8/255, steps=10, n_restarts=1, seed=0, loss='ce', eot_iter=1, rho=.75, verbose=False)
        >>> adv_images = attack(images, labels)
...
class APGDT(Attack):
    ...
    Arguments:
        ...
        n_restarts (int): number of random restarts. (Default: 1)
        seed (int): random seed for the starting point. (Default: 0)
        loss (str): loss function for attack. ['ce', 'drl'] (Default: 'ce')
        eot_iter (int): number of iteration for EOT. (Default: 1)
        rho (float): parameter for step-size update (Default: 0.75)
        verbose (bool): print progress. (Default: False)
    ...
    Shape:
        ...
        - output: :math:`(N, C, H, W)`.
    Examples::
        >>> attack = attack.APGDT(model, norm='Linf', eps=8/255, steps=10, n_restarts=1, seed=0, loss='ce', eot_iter=1, rho=.75, verbose=False, n_classes=10)
        >>> adv_images = attack(images, labels)
    """
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                # loss_indiv = criterion_indiv(logits, y if self.loss == "ce" else y_target)
                loss = loss_indiv.sum()
            # 1 backward pass (eot_iter = 1)
            grad += torch.autograd.grad(loss, [x_adv])[0].detach()
        grad /= float(self.eot_iter)
